...
//process events
void draw() {
  background(vals[0]*255.0,vals[1]*255.0,vals[2]*255.0);
  vals[0] = (sin(millis()/1000.0*TWO_PI/6.)+1.0)/2.0;
  vals[1] = (sin(millis()/1000.0*TWO_PI/1.5)+1.0)/2.0;
  vals[2] = (sin(millis()/1000.0*TWO_PI/3.0)+1.0)/2.0;
  String message = str(vals[0]) + " " + str(vals[1]) + " " + str(vals[2]) + "/n";
  udp.send(message,"localhost",6666);
}
